#!/bin/zsh

# base64 data for script to install ext4fuse:
rbfile=""

# Colors
RED='\033[0;31m'
YELLOW='\033[1;33m'
GREEN='\033[0;32m'
NC='\033[0m'

# Locations to check if dependencies are installed:
macfuse_path="/Library/Filesystems/macfuse.fs"
e2label_path="/usr/local/opt/e2fsprogs/sbin/e2label"

# See if Homebrew is installed:
if ! command -v brew &>/dev/null; then
  # If not, install it.
  echo
  echo " ->> Attempting to install Homebrew... <<-"
  if /bin/bash -c "$(curl -fsSL https://raw.githubusercontent.com/Homebrew/install/HEAD/install.sh)" &>/dev/null; then
    printf "$GREEN%s$NC\n" "<<- Successfully installed Homebrew! ->>"
  else
    printf "$RED%s$NC\n" "X-- Failed to install Homebrew; exiting. --X"
    printf "$RED%s$NC\n" "( Try visiting https://brew.sh and installing manually. )"
    exit 1
  fi
fi

# Make sure homebrew is up to date:
echo
echo " ->> Updating Homebrew... <<-"
brew update

# See if macFUSE is installed:
if [[ ! -d $macfuse_path ]]; then
  # If not, install it.
  echo
  echo " ->> Attempting to install macFUSE... <<-"
  if brew install --cask macfuse &>/dev/null; then
    printf "$GREEN%s$NC\n" "<<- Successfully installed macFUSE! ->>"
  else
    printf "$RED%s$NC\n" "X-- Failed to install macFUSE; exiting. --X"
    printf "$RED%s$NC\n" "( Try visiting https://osxfuse.github.io/ and installing manually. )"
    exit 1
  fi
fi

# See if ext4fuse is installed:
if ! command -v ext4fuse &>/dev/null; then
  # If not, install it.
  echo
  echo " ->> Attempting to install ext4fuse... <<-"
  echo $rbfile | base64 --decode -o ./ext4fuse.rb
  if brew install --formula --build-from-source ./ext4fuse.rb &>/dev/null; then
    rm ./ext4fuse.rb
    printf "$GREEN%s$NC\n" "<<- Successfully installed ext4fuse! ->>"
  else
    rm ./ext4fuse.rb
    printf "$RED%s$NC\n\n" "X-- Failed to install ext4fuse. --X"

    printf "$YELLOW%s$NC\n" "Do you want to attempt installation using Homebrew Fuse XYZ-mac? (Y/N)"
    printf "$YELLOW%s$NC\n" "( Read more in: https://github.com/gromgit/homebrew-fuse )"
    read -r xyz_answer

    if [[ "$xyz_answer" =~ [yY] ]]; then
      brew uninstall XYZ &>/dev/null
      if brew install gromgit/fuse/XYZ-mac &>/dev/null; then
        if brew install ext4fuse-mac &>/dev/null; then
          printf "$GREEN%s$NC\n" "<<- Successfully installed ext4fuse! ->>"
        else
          printf "$RED%s$NC\n" "X-- Failed to install ext4fuse-mac; exiting. --X"
          printf "$RED%s$NC\n" "( Try visiting https://github.com/gerard/ext4fuse and installing manually. )"
          exit 1
        fi
      else
        printf "$RED%s$NC\n" "X-- Failed to install XYZ-mac; exiting. --X"
        printf "$RED%s$NC\n" "( Try visiting https://github.com/gerard/ext4fuse and installing manually. )"
        exit 1
      fi
    fi
  fi
fi

# See if e2fsprogs is installed:
if ! command -v $e2label_path &>/dev/null; then
  # If not, install it.
  echo
  echo " ->> Attempting to install e2fsprogs... <<-"
  if brew install e2fsprogs &>/dev/null; then
    printf "$GREEN%s$NC\n" "<<- Successfully installed e2fsprogs! ->>"
  else
    printf "$RED%s$NC\n" "X-- Failed to install e2fsprogs; exiting. --X"
    printf "$RED%s$NC\n" "( Try visiting https://formulae.brew.sh/formula/e2fsprogs and installing manually. )"
    exit 1
  fi
fi

echo
echo "All dependencies installed!"
echo 'Press any key to exit...'
read -k1 -sr
